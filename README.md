...
- `GET /api/companies/all` — получить список всех компаний и их пользователей
- `GET /api/companies/one/{id}` — получить одну компанию и список её пользователей
- `POST /api/companies` — создать новую компанию
- `PUT /api/companies/{id}` — обновить данные компании
- `DELETE /api/companies/{id}` — удалить компанию (также удаляются все её пользователи)
